...
export class NewOfferPage implements OnInit {
  ...
  onCreateOffer() {
    ...
    const frmData = this.newOfferForm.value;
    this.loadingCtrl
      .create({ message: 'creating place...' })
      .then(loadingEl => {
        loadingEl.present();
        this.placesService.uploadImage(this.newOfferForm.get('image').value).pipe(
          take(1),
          switchMap( uploadRes => {
            ...
            .addPlace(
              ...
              frmData.description,
              +frmData.price,
              new Date(frmData.dateFrom),
              new Date(frmData.dateTo),
              frmData.location,
              uploadRes.imageUrl
            );
          })
